...
jQuery.fn.table2CSV = function(options) {
	...
	var tempGridData = userData.bitacoraGridData.rows; 
	
	for (var key in tempGridData) {
		var row =tempGridData[key];
		var tmpRow = [];
    ...
    function row2CSV(tmpRow, clase) {
        var tmp = tmpRow.join('') // to remove any blank rows
        // alert(tmp);
        if (tmpRow.length > 0 && tmp != '') {
            var mystr = '<tr class="'+clase+'"><td class="nowrap">'+tmpRow.join(options.separator)+'</td></tr>';
    ...
    function popup(data) {
		$("body").append('<form id="exportform" action="controller/plugins/slickgrid/lib/exportExcel.php" method="post"><input type="hidden" id="exportdata" name="exportdata" /></form>');
		$("#exportdata").val(data);
